...
* __具體常數︰__相比於常數變數，一個在上下文中已知且單一值的常數。

* __常數表達式__一個用於評估一個常數的表達式。此可為一個身份表達式或一個在Rust常數系統中可以評估出的更複雜的表達式。
...
常數泛型是一個很大的功能，將需要大量的教育資料，這將會需要在書本跟參考文件，且可能會在書中有獨言的章節。常數泛型的文件化本質上將會是一個大項目。
